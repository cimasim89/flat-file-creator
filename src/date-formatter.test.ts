...
describe('Date formatter execution raise Exception', () => {
  it('If map is null', () => {
    expect(() => rtDateFormatter(null, null)).toThrow('map is null or undefined')
  })
  ...
  it('If field map object not contain size', () => {
    expect(() => rtDateFormatter({ name: 'test', paddingPosition: 'end' }, null)).toThrow(
  ...
  it('If data is not a date', () => {
    const date = ''
    expect(() => rtDateFormatter({ size: 10, name: 'someField' }, date)).toThrow(
      `Invalid date ${date}`
    )
  })
  ...
  it('If padding symbol is more of one char', () => {
    const date = new Date()
    expect(() =>
      rtDateFormatter({ size: 50, name: 'someField', paddingSymbol: '@.@' }, date)
...
describe('String formatter execution result', () => {
  it('size 50 and new Date, result lenght is 50', () => {
    expect(_.size(rtDateFormatter({ size: 50, name: 'someField', type: 'date' }, new Date()))).toBe(50)
  })
  ...
  it(
    ...
    () => {
      expect(rtDateFormatter({ size: 30, name: 'someField', type: 'date' }, '2018-01-01')).toBe(
  ...
  it(
    "size 24 date '2018-01-01'," +
      " result iso string '2018-01-01T00:00:00.000Z'",
    () => {
      expect(rtDateFormatter({ size: 24, name: 'someField', type: 'date' }, '2018-01-01')).toBe(
        '2018-01-01T00:00:00.000Z'
      )
    }
  )
  ...
  it(
    ...
    () => {
      expect(
        rtDateFormatter(
          { size: 10, format: { dateFormat: 'YYYY-MM-DD' }, name: 'someField', type: 'date' },
          '2018-01-01'
        )
      ).toBe('2018-01-01')
    }
  ...
  it(
    ...
    () => {
      expect(
        rtDateFormatter(
          { size: 10, format: { utc: true, dateFormat: 'YYYY-MM-DD' }, name: 'someField', type: 'date' },
          '2018-01-01'
        )
      ).toBe('2018-01-01')
    }
