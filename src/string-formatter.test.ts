...
describe('String formatter execution raise Exception', () => {
  ...
  it('If field map object not contain size', () => {
    expect(() => rtStringFormatter({ paddingPosition: 'end', name: 'someField' }, null)).toThrow(
  ...
  it('If data is not a string and straight option', () => {
    expect(() =>
      rtStringFormatter({ size: 10, name: 'someField', type: 'string', straight: true }, 100)
  ...
  it('If padding position is not allowed', () => {
    ...
    expect(() =>
      rtStringFormatter({ size: 10, name: 'someField', type: 'string', paddingPosition }, '')
...
describe('String formatter execution result', () => {
  it("size 10 and data '', result lenght is 10 ", () => {
    expect(_.size(rtStringFormatter({ size: 10, name: 'someField', type: 'string' }, ''))).toBe(10)
  })

  it("size 10 and long data 'somelongstring', result lenght is 10 ", () => {
    expect(_.size(rtStringFormatter({ size: 10, name: 'someField', type: 'string' }, 'somelongstring'))).toBe(10)
  })

  it("size 10 and long data 'somelongstring', result is 'somelongst'", () => {
    expect(rtStringFormatter({ size: 10, name: 'someField', type: 'string' }, 'somelongstring')).toBe('somelongst')
  })

  it("size 10 and same size data 'somestring', result lenght is 10 ", () => {
    expect(_.size(rtStringFormatter({ size: 10, name: 'someField', type: 'string' }, 'somestring'))).toBe(10)
  })

  it("size 10 and same size data 'somestring', result is 'somestring'", () => {
    expect(rtStringFormatter({ size: 10, name: 'someField', type: 'string' }, 'somestring')).toBe('somestring')
  })

  it("size 10 and spaced data 'str ing', result lenght is 10", () => {
    expect(_.size(rtStringFormatter({ size: 10, name: 'someField', type: 'string' }, 'str ing'))).toBe(10)
  })

  it("size 10 and spaced data 'str ing', result is 'str ing   '", () => {
    expect(rtStringFormatter({ size: 10, name: 'someField', type: 'string' }, 'str ing')).toBe('str ing   ')
  })

  it("paddingPosition 'start' data = 'str ing', result is '   str ing'", () => {
    expect(
      rtStringFormatter({ size: 10, paddingPosition: 'start', name: 'someField', type: 'string' }, 'str ing')
    ).toBe('   str ing')
  })

  it("end paddingSymbol '@' data 'str ing', result is 'str ing@@@'", () => {
    expect(rtStringFormatter({ size: 10, paddingSymbol: '@', name: 'someField', type: 'string' }, 'str ing')).toBe(
      'str ing@@@'
    )
  })
  ...
  it("start paddingSymbol '@' data 'str ing',result is '@@@str ing'", () => {
    expect(
      rtStringFormatter(
        { size: 10, paddingPosition: 'start', paddingSymbol: '@', name: 'someField', type: 'string' },
        'str ing'
      )
    ).toBe('@@@str ing')
  })
